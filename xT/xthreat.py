...
def visualize_xt_heatmap(model: ExpectedThreat, title: str = "Expected Threat (xT) Heatmap") -> None:
    """Visualize the xT heatmap using matplotsoccer.
    ...
    try:
        ...
        print("matplotsoccer is not installed. Please install it to use this function.")
        return

    mps.heatmap(model.xT, cmap="hot", linecolor="white", cbar=True, title=title)


def get_player_contribution(
    model: ExpectedThreat, actions: DataFrame[SPADLSchema]
) -> DataFrame[SPADLSchema]:
    """Calculate xT contribution for each player.
    ...
    -------
    pd.DataFrame
        DataFrame with player_id, player_name, and their total/avg xT contribution.
    """
    # Calculate xT for all actions
    actions_with_xt = actions.copy()
    actions_with_xt["xT_value"] = model.rate(actions_with_xt)

    # Group by player
    # Filter out actions with NaN xT (non-moving actions)
    valid_actions = actions_with_xt.dropna(subset=["xT_value"])
    
    # We need player names, assuming 'player_name' column exists or we merge it later.
    # If 'player_name' is not in actions, we group by 'player_id'.
    group_col = "player_name" if "player_name" in valid_actions.columns else "player_id"

    stats = (
        valid_actions.groupby(group_col)["xT_value"]
        .agg(["sum", "mean", "count"])
        .reset_index()
        .rename(columns={"sum": "total_xT", "mean": "avg_xT", "count": "num_actions"})
    )
    
    return stats.sort_values(by="total_xT", ascending=False)